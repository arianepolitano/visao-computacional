    ...
    if resultado.multi_hand_landmarks:
        for lado_mao, marcacao_maos in zip(resultado.multi_handedness, resultado.multi_hand_landmarks):
            ...
            for marcacao in marcacao_maos.landmark:
                #para alterar as coordenadas para uma escala em pixels, devemos multiplicar as coordenadas pela resolução da imagem.
...
while True: 
    ...
    #faz com que a imagem seja espelhada horizontalmente
    img = cv2.flip(img, 1)

    img, todas_maos = encontra_coordenadas_maos(img)

    #se soh tem uma mao na imagem
    if len(todas_maos) == 1:
        ...
        if todas_maos[0]['lado'] == 'Right':
            ...
            if info_dedos_mao1 == [False, False, True, True] and ferramenta_captura == False:
                ferramenta_captura = True
